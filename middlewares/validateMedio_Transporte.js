...
const validarMedioTransporte = [
  ...
  body("nombre")
    ...
    .custom(async (nombre, { req }) => {
      ...
      if (!empresaId) {
        throw new Error(
          "El ID de la empresa es requerido para validar el nombre del transporte."
        );
      }
      ...
      if (
        transporteExistente &&
        transporteExistente.id !== parseInt(req.params.id)
      ) {
        throw new Error(
          "Ya existe un transporte con ese nombre en la misma empresa."
        );
      }
  ...
  body("patente")
    ...
    .isString()
    .withMessage("La patente debe ser un string.")
    .matches(/^[A-Z]{2}\d{3}[A-Z]{2}$/)
    .withMessage(
      "El formato de la patente debe ser de dos letras, tres números y dos letras en mayúsculas."
    )
    .custom(async (patente, { req }) => {
      ...
      if (transporte && transporte.id !== parseInt(req.params.id)) {
        throw new Error(
          "La patente ya está en uso por otro medio de transporte"
        );
      }
      return true;
    }),
  ...
  body("cantLugares")
    .notEmpty()
    .withMessage("La cantidad de lugares es requerida.")
    .isInt({ min: 10 })
    .withMessage("La cantidad de lugares debe ser un número mayor a 10."),
