  ...
  try {
    ...
    if (!origen || !destino) {
      return res
        .status(400)
        .json({
          error: "Por favor, proporciona tanto el origen como el destino.",
        });
    }
    ...
    const viajesDisponibles = todosLosViajes.filter((viaje) => {
      ...
      return (
        fechaViajeLimpia >= fechaActualLimpia &&
    ...
    if (viajesDisponibles.length === 0) {
      return res
        ...
        .json({
          error:
            "No hay viajes disponibles para el origen y destino especificados.",
        });
    }
