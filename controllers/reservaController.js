...
exports.obtenerReservasPorEmpresa = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener las reservas por empresa" });
  }
};
...
exports.crearReserva = async (req, res) => {
  try {
    ...
    const medioTransporte = await medioTransporteController.obtenerTransporteId(
      viaje.medioTransporte_id
    );
    if (!medioTransporte) {
      return res
        .status(404)
        .json({ mensaje: "Medio de transporte no encontrado" });
    }
    ...
    if (viaje.cantPasajeros < personas.length) {
      return res.status(400).json({
        mensaje: "No hay suficientes lugares disponibles en este viaje",
      });
    }

    // Verificar si el usuario ya tiene una reserva para este viaje
    const usuarioReserva = await reservaUsuario.obtenerReservaPorUsuarioYViaje(
      usuarios_id,
      viajes_id
    );
    if (usuarioReserva) {
      return res
        .status(400)
        .json({ mensaje: "El usuario ya posee una reserva para este viaje" });
    }
    ...
    res
      .status(201)
      .json({ message: "Reserva creada exitosamente", reserva: nuevaReserva });
  } catch (error) {
    console.error(error);
...
exports.actualizarReserva = async (req, res) => {
  try {
    const { nombre, apellido, dni, ubicacionOrigen, ubicacionDestino } =
      req.body;
    const fechaActual = new Date();
    ...
    if (actualizar === 0) {
      return res
        .status(200)
        .json({ message: "No hubo cambios en los datos del pasajero" });
    }
...
exports.eliminarReserva = async (req, res) => {
  try {
    ...
    if (!medioTransporte) {
      return res
        .status(404)
        .json({ mensaje: "Medio de transporte no disponible" });
    }
...
exports.eliminarPasajero = async (req, res) => {
  try {
    ...
    const totalpasajeros = await Pasajeros.findAll({
      where: { reserva_id: pasajeros.reserva_id, eliminado: "no" },
    });
...
exports.listarPasajerosPorReserva = async (req, res) => {
  try {
    ...
    if (!pasajeros) {
      return res
        .status(404)
        .json({ error: `No se encontraron pasajeros para la reserva ` });
    }
    ...
    res
      .status(500)
      .json({ error: "Error al obtener los pasajeros de la reserva" });
  }
};
...
exports.listarPasajeroPorId = async (req, res) => {
  try {
    ...
    if (!pasajeros) {
      return res
        .status(404)
        .json({ error: `No se encontro pasajero para ese id` });
    }
...
exports.obtenerReservasPorUsuario = async (req, res) => {
  try {
    ...
    const reservas = await Reserva.findAll({
      attributes: ["id", "fechaReserva", "usuarios_id", "viajes_id"],
      where: { usuarios_id: usuarioId, eliminado: "no" },
...
exports.listarPasajerosPorViaje = async (req, res) => {
  try {
    ...
    if (!reservas || reservas.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron reservas para este viaje." });
    }
    ...
    res
      .status(500)
      .json({ error: "Error al obtener los pasajeros del viaje." });
  }
};
...
exports.listarReservasYPasajerosPorViaje = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener las reservas y pasajeros del viaje." });
  }
};
