...
exports.obtenerViajesPorChofer = async (req, res) => {
  try {
    ...
    if (viajes.length === 0) {
      return res
        .status(200)
        .json({ message: "El chofer no posee viajes", viajes: [] });
    }
...
exports.crearViaje = async (req, res) => {
  try {
    ...
    if (!transporte) {
      return res
        .status(400)
        .json({ error: "El medioTransporte_id no es válido" });
    }
    ...
    if (!usuarioEmpresa) {
      return res.status(400).json({
        error:
          "El usuarioEmpresa_id no es válido o el usuario no tiene perfil de chofer",
      });
    }
    ...
    if (empresaUsuarioId !== empresaTransporteId) {
      return res.status(400).json({
        error:
          "El usuario no pertenece a la misma empresa que el medio de transporte",
      });
    }
    ...
    res
      .status(201)
      .json({ message: "Viaje creado exitosamente", viaje: nuevoViaje });
  } catch (error) {
    console.error(error);
...
exports.eliminarViajes = async (req, res) => {
  try {
    ...
    if (reservasAsociados) {
      return res.status(400).json({
        error: "No se puede eliminar el viaje porque tiene reservas asignadas.",
      });
    }
