...
exports.asociarUsuarioEmpresa = async (req, res) => {
  try {
    ...
    if (!usuario || !empresa) {
      return res
        .status(404)
        .json({ mensaje: "Usuario o empresa no encontrados" });
    }
    ...
    if (asociacionExistente) {
      return res.status(400).json({
        mensaje:
          "El usuario ya está asociado a otra empresa. Debe desvincularse primero.",
      });
    }
    ...
    res
      .status(200)
      .json({ mensaje: "Usuario asociado a la empresa correctamente" });
  } catch (error) {
    console.error(error);
...
exports.desasociarUsuarioEmpresa = async (req, res) => {
  try {
    ...
    if (!asociacion) {
      return res
        .status(400)
        .json({ mensaje: "El usuario no está asociado a ninguna empresa" });
    }
    ...
    res
      .status(200)
      .json({ mensaje: "Usuario desasociado de la empresa correctamente" });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ mensaje: "Error al desasociar usuario de la empresa" });
  }
};
  ...
  try {
    ...
    if (!asociacion) {
      return res.status(200).json({
        mensaje: "El usuario no está asociado a ninguna empresa",
        empresa_id: null,
      });
    }
...
exports.obtenerUsuariosPorEmpresa = async (req, res) => {
  ...
  try {
    ...
    if (!usuariosEmpresa || usuariosEmpresa.length === 0) {
      return res
        .status(404)
        .json({ mensaje: "No hay usuarios asociados a la empresa" });
    }
