...
exports.crearVenta = async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Error al contar los pasajeros de la reserva" });
    }
    ...
    const horaString = fechaActual.toLocaleTimeString("es-ES", {
      hour12: false,
    });
    ...
    const nuevaVenta = await Ventas.create(
      ...
      {
        where: { reserva_id: reserva_id, eliminado: "no" },
      }
    );
...
exports.eliminarVentas = async (req, res) => {
  try {
    ...
    const [eliminar] = await Ventas.update(
      { eliminado: "si" },
      {
        where: { id: req.params.id },
        fields: ["eliminado"],
...
exports.obtenerVentaDetalleGeneral = async (req, res) => {
  try {
    ...
    const pasajeros = await reservaController.listarPasajerosReserva(
      req.params.id
    );

    const transporte = await medioTransporteController.obtenerTransporteId(
      viaje.medioTransporte_id
    );
    const empresa = await empresaController.obtenerEmpresaPorId(
      transporte.empresa_id
    );
